...
Official LaLiga Fantasy has no website JSON. Copy `collector/templates/laliga-fantasy-oficial.xlsx`, fill **Competition** and **Squads** from the Android app, delete the yellow EXAMPLE rows, then `import-excel`. That is a separate dashboard competition from SofaScore LaLiga.
