...
def overlay_from_lineups(payload: dict[str, Any]) -> tuple[dict[str, float], set[str]]:
    """Parse event/{id}/lineups JSON into per-player ratings and injured (missing) ids."""
    ...
    *,
    ...
    for event in events:
        ...
        if lineups.status_code != 200:
            continue
        event_ratings, event_injured = overlay_from_lineups(lineups.json() or {})
...
def annotate_snapshot_injuries(snapshot: Snapshot) -> Snapshot:
    """Mark Injured from this GW's SofaScore missingPlayers, plus current injuries on live weeks."""
    ...
    try:
        ...
        overlay = {"ratings": {}, "injuredIds": set()}
    ...
    if status in {"live", "upcoming"}:
        ...
        try:
            injured |= fetch_injured_player_ids(team_ids)
        except Exception:
            pass
    return apply_round_overlay(snapshot, injured_ids=injured)
